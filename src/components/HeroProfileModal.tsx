  ...
  return (
    ...
    >
      <div
        ...
        className="relative max-w-4xl w-full bg-[#0d071d]/95 border-2 border-yellow-500/60 rounded-3xl p-6 sm:p-8 shadow-[0_20px_60px_rgba(0,0,0,0.95)] flex flex-col md:flex-row items-center gap-8"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="relative w-72 h-96 rounded-3xl overflow-hidden border-2 border-yellow-500/80 shadow-[0_0_35px_rgba(234,179,8,0.3)] bg-black/90 flex-shrink-0 group">
          ...
          <div className="absolute top-3 left-3 bg-black/80 border border-yellow-500/50 rounded-full px-3 py-1 text-[11px] font-bold text-yellow-300 font-cinzel shadow">
            {profile.difficulty}
          </div>

          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
            <span className="text-xs font-bold text-emerald-400 bg-emerald-950/90 px-3 py-1 rounded-full border border-emerald-500/60 shadow">
              ❤️ {hero.hp} Health
            </span>
            <span className="text-xs text-yellow-300 bg-yellow-950/90 px-2.5 py-1 rounded-full border border-yellow-500/50 shadow font-cinzel">
              ★ COMMANDER
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-between h-full space-y-4">
          <div>
            <div className="mb-2">
              <h2 className="font-cinzel text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-200 to-amber-500 tracking-wider">
                {hero.name}
              </h2>
